; Verificacion: Archivo modificado correctamente
...
.DATA
    ...
    MAX_INPUT_LEN       DB 8            ; Longitud maxima de entrada (8 caracteres para DOS)
    ...
    TXT_CHEATSHEET      DB 'Alt+S:Guard Alt+Z:Menu Alt+B:Busq Alt+H:Ayu', 0
    ...
    TXT_H_ED            DB 'Flechas: Moverse | Alt+Z: Menu | BS: Borrar', 0
...
RM_WAIT_KEY:
    ...
    ; Atajo Alt+X (Scan Code 2Dh con AL=0) para salida directa
    CMP AL, 0
    JNE RM_NOT_ALT_X
    CMP AH, 2DH
    JE  RM_EXIT_ACTION
...
RSI_KEY_LOOP:
    ...
    ; Alt+Z (Scan Code 2Ch con AL=0) -> Cancelar
    CMP AL, 0
    JNE RSI_CHK_ENTER
    CMP AH, 2CH
    JNE RSI_CHK_ENTER
...
RSI_ADD_CHAR:
    PUSH AX
    MOV AL, MAX_INPUT_LEN
    MOV AH, 0
    CMP BX, AX              ; Limitar segun MAX_INPUT_LEN (8 para DOS, 24 para busqueda)
    POP AX
    JAE RSI_KEY_LOOP
...
FCN_RETRY:
    ...
    ; Leer nombre de archivo (Mayusculas obligatorias, maximo 8 caracteres para DOS)
    MOV FORCE_UPPER, 1
    MOV MAX_INPUT_LEN, 8
...
FOF_RETRY:
    ...
    ; Leer nombre de archivo (Normalizado a mayusculas, maximo 8 caracteres para DOS)
    MOV FORCE_UPPER, 1
    MOV MAX_INPUT_LEN, 8
    CALL READ_STRING_INPUT
    CMP AX, 0FFFFH
...
DEK_CHECK_ALT_OR_ARROWS:
    ...
    ; Atajos Alt del Editor (Requieren obligatoriamente tecla Alt: AL=0)
    CMP AH, 2CH             ; Alt+Z: Volver al menu principal
    JE  DEK_ALT_Z
...
ACTION_SEARCH_REPLACE PROC NEAR
    ...
    ; Permitir mayusculas y minusculas exactas en busqueda (max 24 car)
    MOV FORCE_UPPER, 0
    MOV MAX_INPUT_LEN, 24
    CALL READ_STRING_INPUT
    CMP AX, 0FFFFH
    JE  ASR_CANCEL
    CMP AX, 0
